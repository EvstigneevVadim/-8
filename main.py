...
class SegmentApp:
    def __init__(self, root):
        ...
        tk.Button(btn_frame, text="Сегментация по длине", command=self.segment_by_length, **btn_style).grid(row=1,
                                                                                                            column=0,
                                                                                                            pady=3)
        tk.Button(btn_frame, text="Визуализировать", command=self.visualize, **btn_style).grid(row=2, column=0, pady=3)
        tk.Button(btn_frame, text="Переместить отрезки", command=self.move_all, **btn_style).grid(row=3, column=0,
                                                                                                  pady=3)
    ...
    def show_groups_info(self):
        """Показывает информацию о группах в отдельном окне"""
        if not self.groups:
            return

        info_win = tk.Toplevel(self.root)
        info_win.title("Информация о группах отрезков")
        info_win.geometry("400x300")

        # Текстовое поле с прокруткой
        text_widget = tk.Text(info_win, wrap=tk.WORD, padx=10, pady=10, font=("Arial", 11))
